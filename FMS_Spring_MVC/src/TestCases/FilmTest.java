...
public class FilmTest {
	...
	FilmService service = ctx.getBean("fservice",FilmServiceImpl.class);
	@Test
	...
	public void creatingFilmSuccess() throws Exception{
		...
		f.setActors(actors);
		Album al = new Album();
		PhotoCollection ph = new PhotoCollection();
		ph.setPhoto_url("vamsi");
		al.getPhotocollection().add(ph);
		f.setAlbum(al);
		Date d = new Date();
		f.setLast_update(new Timestamp(d.getTime()));;
		assertTrue(service.saveFilm(f));
	}
	@Test
	public void modifyFilmSuccess() throws Exception{
		Film f = new Film();
		f.setTitle("KRISHsdfH");
		
		f.setRating(Rating.G);
		f.setRelease_year(2015);;
	
		assertTrue(service.changeFilm("Bahubali",f));
	}
	@Test
	public void removeFilm() throws Exception{
		assertTrue(service.deleteFilm("WYOMING STORM"));
	}
	@Test
